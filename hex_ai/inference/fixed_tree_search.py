...
class FixedTreeSearch:
    ...
    def _check_memory_usage(self, verbose: int):
        ...
        # Use a higher threshold and only warn once per search to avoid flooding
        if memory_mb > 5000:  # 5GB threshold
...
def minimax_policy_value_search_with_batching(
    ...
    stats = {
        ...
        'policy_evaluations': result.stats.get('policy_evaluations', 0),
        'value_evaluations': result.stats.get('value_evaluations', 0),
        'policy_batches': result.stats.get('policy_batches', 0),
        'value_batches': result.stats.get('value_batches', 0),
        'policy_nn_time': result.stats.get('policy_nn_time', 0.0),
        'value_nn_time': result.stats.get('value_nn_time', 0.0),
        'memory_usage_mb': result.stats.get('memory_usage_mb', 0.0),
    }
