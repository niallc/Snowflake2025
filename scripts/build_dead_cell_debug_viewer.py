#!/usr/bin/env python3
"""Build a static HTML viewer for dead-cell root-pruning debug JSONL logs."""
...
def _load_records(path: Path, max_records: int) -> List[Dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_no, raw_line in enumerate(handle, start=1):
            ...
            if payload.get("event_type") != "dead_cell_prune_root":
                continue
...
<head>
  ...
  <title>Dead-Cell Debug Viewer</title>
  <style>
    :root {{
      --bg: #f6f5ef;
      --fg: #1d1c19;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .cell.dead {{ outline: 3px solid var(--dead); outline-offset: 1px; }}
    .meta {{
      margin-top: 14px;
      border-top: 1px solid var(--line);
      padding-top: 12px;
...
<body>
  <div class="layout">
    <section class="panel">
      <h1 class="title">Dead-Cell Debug Viewer</h1>
  ...
  <script>
    ...
    const allRules = Array.from(new Set(records.flatMap((r) => r.rules || []))).sort();
    ...
    globalStatsEl.textContent = `Loaded ${{records.length}} records | Rules: ${{allRules.join(", ") || "(none)"}}`;
    ...
    function buildBoardHtml(entry) {{
      ...
      const deadRow = Number(entry.dead_cell?.row);
      const deadCol = Number(entry.dead_cell?.col);
      ...
      for (let r = 0; r < parsed.boardSize; r++) {{
        ...
        for (let c = 0; c < parsed.boardSize; c++) {{
          ...
          if (r === deadRow && c === deadCol) classes.push("dead");
    ...
    function buildMetaHtml(entry) {{
      const deadCellLabel = entry.dead_cell?.trmph || `(${{entry.dead_cell?.row}},${{entry.dead_cell?.col}})`;
      const rules = (entry.rules || []).join(", ");
      const trmphHref = `https://trmph.com/hex/board${{entry.state_trmph || ""}}`;
      return `
        <div class="meta">
          <div class="meta-line"><span class="meta-key">Dead Cell:</span>${{escapeHtml(deadCellLabel)}}</div>
          ...
          <div class="meta-line"><span class="meta-key">Pruned Moves (root):</span>${{escapeHtml(entry.pruned_moves_total)}} / legal before ${{escapeHtml(entry.legal_moves_before_count)}}</div>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a static HTML viewer from dead-cell debug JSONL logs."
    )
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
        help="Path to dead-cell debug JSONL log produced by run_tournament dead-cell debug mode.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    if not records:
        raise ValueError(
            f"No dead-cell root-prune records found in {args.input}. "
